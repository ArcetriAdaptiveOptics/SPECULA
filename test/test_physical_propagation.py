...
class Test(unittest.TestCase):
    ...
    def test_physicalProp(self, target_device_idx, xp):
        ...
        uplink_source = Source(polar_coordinates=[0.0, 0.0], magnitude=0, height=200, wavelengthInNm=1550)
        downlink_source = Source(polar_coordinates=[0.0, 0.0], magnitude=0, height=200, wavelengthInNm=1550)
    ...
    def test_physicalProp_padding(self, target_device_idx, xp):
        ...
        seeing = WaveGenerator(constant=1.0, target_device_idx=target_device_idx)
        wind_speed = WaveGenerator(constant=[0, 0, 0], target_device_idx=target_device_idx)
        wind_direction = WaveGenerator(constant=[0, 0, 0], target_device_idx=target_device_idx)
        ...
        downlink_phase1 = unwrap_phase(cpuArray(prop_down1.outputs['out_downlink_source_ef'].phaseInNm),
                                       wrap_around=[True, True], rng=1)
        downlink_phase2 = unwrap_phase(cpuArray(prop_down2.outputs['out_downlink_source_ef'].phaseInNm),
                                       wrap_around=[True, True], rng=1)
    ...
    def test_physicalProp_bandlimit(self, target_device_idx, xp):
        ...
        uplink_phase1 = unwrap_phase(cpuArray(prop_up1.outputs['out_uplink_source_ef'].phaseInNm),
                                     wrap_around=[True, True], rng=1)
        uplink_phase2 = unwrap_phase(cpuArray(prop_up2.outputs['out_uplink_source_ef'].phaseInNm),
                                     wrap_around=[True, True], rng=1)

        rms = xp.sqrt(
            xp.mean((uplink_phase1 / np.max(uplink_phase1) - uplink_phase2 / np.max(uplink_phase2)) ** 2))
        print(rms)
        self.assertTrue(rms < 0.1)
