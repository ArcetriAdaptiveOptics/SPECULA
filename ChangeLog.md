...
- Added per-object timing of all trigger phases: NVTX ranges for Nsight Systems, and new `--trace-file`, `--trace-skip`, `--trace-sync` and `--trace-gpu-events` options to write them to a text file with a summary (see docs/profiling.rst). The tracer can also be used as a context manager or decorator to mark more sections of code; `show_in_profiler()` is now an alias for it. Also fixed `--profile` failing.
