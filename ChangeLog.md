...
- `ModalAnalysis.trigger_code()` no longer projects the single `in_ef` input twice (once for `out_modes`, then again in the per-input loop, which already writes `out_modes`). Identical outputs, ~1.6x faster per call (e.g. 0.77 -> 0.45 ms with phase unwrapping at 80x80 pixels, 100 modes). This also removes an unconditional access to `self.in_ef`, which is `None` when using `in_ef_list`.
