...
class Simul():
    ...
    def trigger_order(self, params_orig):
        ...
        Special handling for :-1 syntax:
        - Input with :-1 becomes output of consumer
        - Output of provider becomes artificial input of provider
        - This swaps the dependency direction for ordering
        '''
        order = []
        order_index = []
        ii = 0
        ...
        # Swap input/output for delayed dependencies
        params = self._swap_delayed_dependencies(params)
    ...
    def _swap_delayed_dependencies(self, params):
        """
        Swap input/output for delayed dependencies to reverse ordering.

        If prop uses dm.out_layer:-1:
        1. Remove dm.out_layer:-1 from prop inputs
        2. Add out_layer to prop outputs  
        3. Add _artificial: prop.out_layer to dm inputs

        This makes dm depend on prop for ordering purposes.
        """
        params_modified = deepcopy(params)
        swaps_made = []

        self.verbose = True  # Enable verbose output for debugging

        # Find all delayed dependencies and collect swaps
        for consumer_name, consumer_config in params.items():
            if not isinstance(consumer_config, dict) or 'inputs' not in consumer_config:
                continue

            inputs_to_remove = []
            outputs_to_add = []

            for input_name, input_source in consumer_config['inputs'].items():
                if isinstance(input_source, str) and ':-1' in input_source:
                    # Parse the delayed input
                    provider_output = input_source.split(':-1')[0]  # "dm.out_layer"
                    provider_name = self.output_owner(provider_output)
                    output_name = provider_output.split('.')[-1]  # "out_layer"

                    inputs_to_remove.append(input_name)
                    outputs_to_add.append(output_name)
                    swaps_made.append((consumer_name, provider_name, input_name, output_name))

                    if self.verbose:
                        print(f"  Swapping: {consumer_name} uses {provider_output}:-1")
                        print(f"    Remove input: {consumer_name}.{input_name}")
                        print(f"    Add output: {consumer_name}.{output_name}")
                        print(f"    Add artificial input: {provider_name}._artificial_{output_name}")

                elif isinstance(input_source, list):
                    # Handle lists containing delayed inputs
                    new_input_list = []
                    for item in input_source:
                        if isinstance(item, str) and ':-1' in item:
                            # Parse the delayed input
                            provider_output = item.split(':-1')[0]  # "dm.out_layer"
                            provider_name = self.output_owner(provider_output)
                            output_name = provider_output.split('.')[-1]  # "out_layer"
                            ...
                            if self.verbose:
                                print(f"  Swapping (from list): {consumer_name} uses {provider_output}:-1")
                                print(f"    Add output: {consumer_name}.{output_name}")
                                print(f"    Add artificial input: {provider_name}._artificial_{output_name}")
                        else:
                            new_input_list.append(item)

                    # Update the input list (remove delayed items)
                    if len(new_input_list) != len(input_source):
                        if new_input_list:
                            params_modified[consumer_name]['inputs'][input_name] = new_input_list
                        else:
                            inputs_to_remove.append(input_name)

            # Apply changes to consumer
            for input_name in inputs_to_remove:
                del params_modified[consumer_name]['inputs'][input_name]

            if outputs_to_add:
                if 'outputs' not in params_modified[consumer_name]:
                    params_modified[consumer_name]['outputs'] = []
                params_modified[consumer_name]['outputs'].extend(outputs_to_add)

        # Add artificial inputs to providers
        for consumer_name, provider_name, input_name, output_name in swaps_made:
            if provider_name in params_modified:
                if 'inputs' not in params_modified[provider_name]:
                    params_modified[provider_name]['inputs'] = {}

                # Create unique artificial input name
                artificial_input = f'_artificial_{output_name}'
                counter = 0
                while artificial_input in params_modified[provider_name]['inputs']:
                    counter += 1
                    artificial_input = f'_artificial_{output_name}_{counter}'

                params_modified[provider_name]['inputs'][artificial_input] = f'{consumer_name}.{output_name}'

                if self.verbose:
                    print(f"    Added: {provider_name}.{artificial_input} = {consumer_name}.{output_name}")

        if swaps_made and self.verbose:
            print(f"Swapped {len(swaps_made)} delayed dependencies for ordering")

        self.verbose = False  # Reset verbose flag after processing
